...
import { MESSAGE_METHOD_ENUM } from "src/constant/constant";
import emailHandler from "src/core/email/emailHandler";
import smsHandler from "src/core/sms/smsHandler";

const configHelper = {
  async sendMessage(subject: string, content: string, to: string, messageMethodNum: MESSAGE_METHOD_ENUM) {
    return (messageMethodNum === MESSAGE_METHOD_ENUM.EMAIL)
